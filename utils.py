...
from tom_models import DialogueTurn
...
def build_tom_annotation(
    turn_index: int, turn: DialogueTurn
    ...
    return {
        ...
        "temporal_trajectory": {
            ...
            "changes_from_previous": trajectory.changes_from_previous
            if trajectory
            else {},
            "causal_event": {
            ...
            "temporal_chain": [link.to_dict() for link in trajectory.temporal_chain]
            if trajectory
            else [],
            "anchored_history": trajectory.anchored_history if trajectory else [],
        },
        "temporal_chain_reasoning": [
            link.to_dict() for link in tom.temporal_chain_reasoning
        ],
        "tom_errors_detected": [
            {
                ...
                "original_value": truncate_text(str(e.original_value))
                if e.original_value
                else None,
                "corrected_value": truncate_text(str(e.corrected_value))
                if e.corrected_value
                else None,
            }
