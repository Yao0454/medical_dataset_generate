...
class Config:
    ...
    def from_args(
        ...
        config.llm.local_model_path = local_model_path or os.environ.get(
            "LOCAL_MODEL_PATH", ""
        )
        config.llm.device = device
        config.llm.load_in_8bit = load_in_8bit
        config.llm.load_in_4bit = load_in_4bit
        config.llm.tensor_parallel_size = tensor_parallel_size
        config.llm.gpu_memory_utilization = gpu_memory_utilization
        if delay is not None:
            config.llm.delay = delay
