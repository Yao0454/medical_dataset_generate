...
class MedicalDatasetGenerator:
    ...
    def extract_patient_info(self, ehr_data: Dict) -> Dict[str, Any]:
        ...
        demographics_match = re.search(
            r"## Patient Demographics \[None\](.*?)(?=##|\Z)", input_text, re.DOTALL
        )
        ...
        triage_match = re.search(
            r"## Triage \[.*?\](.*?)(?=##|\Z)", input_text, re.DOTALL
        )
        if triage_match:
        ...
        discharge_match = re.search(
            r"## Discharge \[.*?\](.*?)(?=##|\Z)", input_text, re.DOTALL
        )
        if discharge_match:
            ...
            if allergies_match:
                ...
                patient_info["allergies"] = [
                    a.strip() for a in allergies_text.split("/") if a.strip()
                ]
        ...
        medrecon_match = re.search(
            r"## Medrecon \[.*?\](.*?)(?=##|\Z)", input_text, re.DOTALL
        )
        if medrecon_match:
            medrecon_text = medrecon_match.group(1)
            med_matches = re.findall(
                r"\|\s*([A-Za-z0-9\s\-\[\]]+?)\s*\|", medrecon_text
            )
            patient_info["medications"] = [
                m.strip()
                for m in med_matches
                if m.strip() and m.strip() not in ["Name", "Atc Type"]
            ][:10]
    ...
    def generate_doctor_response_with_tom(
        ...
        if not tom_reasoning.has_valid_data():
            logger.warning(
                "ToM reasoning has no valid data, generating fallback response"
            )
...
"""
    ...
    def _generate_fallback_doctor_response(
        self, dialogue_history: List[DialogueTurn], task_type: str
    ) -> str:
        import random
        ...
        if task_type == TaskType.DIAGNOSIS.value:
            ...
            return (
                random.choice(candidates) if candidates else prescription_responses[0]
            )

    def _check_dialogue_repetition(
        self, dialogue: List[DialogueTurn], new_content: str, role: str
    ) -> bool:
        if len(dialogue) < 2:
            return False

        same_role_contents = [turn.content for turn in dialogue if turn.role == role]
    ...
    def _should_terminate_early(self, dialogue: List[DialogueTurn]) -> bool:
        if len(dialogue) < 4:
            return False
    ...
    def generate_dialogue_with_tom(
        ...
        dialogue.append(
            DialogueTurn(content=initial_doctor_msg, role="assistant", turn_number=0)
        )

        should_invoke, dom_level, decision_reason = (
            self.tom_module.step1_tom_invocation_decision(context, dialogue, task_type)
        )
        ...
        for turn_num in range(1, max_turns):
            if self._should_terminate_early(dialogue):
                logger.warning(
                    f"Early termination at turn {turn_num} due to repetition"
                )
                break
            ...
            if self._check_dialogue_repetition(dialogue, patient_response, "user"):
                logger.warning(
                    f"Patient response repetition detected at turn {turn_num}"
                )

            dialogue.append(
                DialogueTurn(
                    content=patient_response, role="user", turn_number=turn_num * 2 - 1
                )
            )
            ...
            goal_achieved, reason, goal_status = (
                self.goal_checker.check_tom_goal_achieved(
            ...
            if self._check_dialogue_repetition(dialogue, doctor_response, "assistant"):
                logger.warning(
                    f"Doctor response repetition detected at turn {turn_num}"
                )
    ...
    def _generate_final_response(
        ...
        # 修复：注入 EHR 患者信息，防止模型瞎编
        prompt = f"""{system_prompt}

    === PATIENT MEDICAL RECORD (CRUCIAL) ===
    {json.dumps(context.get("patient_info", {}), ensure_ascii=False)}
    ...
    3. Shows empathy and ensures patient understanding
    ...
    def generate_single_sample(
        ...
        dialogue, tom_reasonings = self.generate_dialogue_with_tom(
            patient_info, task_type
        )
